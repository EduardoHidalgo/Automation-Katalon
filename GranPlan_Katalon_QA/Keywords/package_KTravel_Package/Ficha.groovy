 ...
 * el servicio de "Paquetes" de KTravel y seleccionar una habitación.
 *
...
public class Ficha {
	...
	{	
		...
		//'return $($("#HotelRatesBox").find("input[type='button']").first()).length
		String query = "return \$(\$(\"#HotelRatesBox\").find(\"input[type='button']\").first()).length";
		...
		else
		{
			...
			WebUI.executeJavaScript("\$(\$(\"#HotelRatesBox\").find(\"input[type='button']\").first()).click()", null);
			//Validacion -> espera a que la pagina haya cargado correctamente
			WebUI.waitForPageLoad(5, FailureHandling.CONTINUE_ON_FAILURE)
