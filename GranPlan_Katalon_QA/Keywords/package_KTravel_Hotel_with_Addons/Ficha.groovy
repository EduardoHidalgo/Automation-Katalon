 ...
 * el servicio de "Hoteles" de KTravel y seleccionar una habitación.
 *
...
public class Ficha {
	...
	public static SeleccionarHabitacion() {
		...
		//$($("#HotelProducts").find("input[type='submit']")).first().length
		String query = "return \$(\$(\"#HotelProducts\").find(\"input[type='submit']\").first()).length";
		...
		else
		{
			...
			WebUI.executeJavaScript("\$(\$(\"#HotelProducts\").find(\"input[type='submit']\").first()).click()", null);
			//Validacion -> espera a que la pagina haya cargado correctamente
			WebUI.waitForPageLoad(5, FailureHandling.CONTINUE_ON_FAILURE)
